...
function files(dir, suffix) {
 return fs.readdirSync(dir, { withFileTypes: true }).flatMap(entry => {
  ...
  return entry.isDirectory() ? files(full, suffix) : suffix.test(full) ? [full] : [];
 });
}
...
function suspiciousText(value) {
 ...
 const reasons = [];
 if (/\b(?:you (?:can|may|must|have)|the target|on a (?:success|failure)|until (?:your|their)|spend a hope|mark a stress|once per (?:scene|session|rest))\b/i.test(value)) reasons.push('English rule phrase');
 if (/\b(?:[Vv]erwenden|[Mm]arkieren|[Gg]eben|[Mm]achen|[Bb]itten|[Hh]andeln) Sie\b/.test(value)) reasons.push('inconsistent formal imperative');
 if (/ZXQ|\uFFFD|\(Deutscher\)/.test(value)) reasons.push('damaged text or translation artifact');
 return reasons;
}

function audit() {
 const findings = [];
 const add = (file, location, reason, text) => findings.push({file:path.relative(root,file),location,reason,text});
 for (const dir of ['src/data/srd/locales/de','src/data/locales/de']) {
  for (const file of files(path.join(root,dir), /\.json$/)) {
   function visit(value, location) {
    if(typeof value === 'string') {
     for(const reason of suspiciousText(value)) add(file,location,reason,value);
 ...
 const names = new Set(['DaggerForge','Daggerheart','Daggerheart © Darrington Press 2025']);
 for(const dir of ['src/features','src/utils']) {
  for(const file of files(path.join(root,dir), /\.tsx?$/)) {
...
if(require.main === module) {
 ...
 if(process.argv.includes('--json')) console.log(JSON.stringify(findings,null,2));
  ...
  for(const f of findings) console.log(`${f.file}:${f.location}: ${f.reason}: ${f.text.slice(0,180)}`);
  console.log(`${findings.length} findings requiring review. Dynamic templates and semantic mistranslations require manual review.`);
 }
