 ...
 *  in the plugin; the character only stores its marks. */
...
export interface CompanionData {
	...
	damageDie: string;
	/** Marks per training option, keyed like "vicious". */
...
export function defaultCompanion(): CompanionData {
	return {
		...
		art: "",
		evasion: "",
		stress: new Array<boolean>(COMPANION_STRESS_SLOTS).fill(false),
...
export function createEmptyCharacter(id: string): CharacterData {
	return {
		...
		evasion: "",
		armorScore: "",
		armorSlots: new Array<boolean>(ARMOR_SLOTS).fill(false),
		majorThreshold: "",
		severeThreshold: "",
		hp: new Array<boolean>(HP_SLOTS).fill(false),
		stress: new Array<boolean>(STRESS_SLOTS).fill(false),
		hope: new Array<boolean>(HOPE_SLOTS).fill(false),
		hopeFeature: "",
		experiences: Array.from({ length: EXPERIENCE_ROWS }, () => ({ text: "", modifier: "" })),
		goldHandfuls: new Array<boolean>(GOLD_HANDFULS).fill(false),
		goldBags: new Array<boolean>(GOLD_BAGS).fill(false),
		goldChest: false,
		classFeature: "",
		proficiency: new Array<boolean>(PROFICIENCY_SLOTS).fill(false),
		weaponHandOne: false,
		weaponHandTwo: false,
