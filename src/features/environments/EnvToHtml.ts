...
<span class="df-env-feat-name">${f.name}</span> - <span class="df-env-feat-type">${gameTerm(f.type)}:</span> ${costHTML}
</div>
...
<button class="df-env-edit-button" data-edit-mode-only="true" aria-label="${t("embed.edit")}" id="${hiddenID}"><svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg></button>
${countdownsHtml}<div class="df-env-name" id="${hiddenID}">${env.name}</div>
<div class="df-env-feat-tier-type">${t("ui.tier")} ${env.tier.toString()} ${gameTerm(env.type)} <span class="df-source-badge-${(env.source || "core").toLowerCase()}">${(env.source || "core").toLowerCase()}</span></div>
<p class="df-env-desc">${env.desc}</p>
<p class="df-env-impulse-line"><span class="df-env-impulse-label">${t("embed.impulse")}:</span> ${env.impulse || ""}</p>
<div class="df-env-card-diff-pot">
<p class="df-env-diff-line"><span class="df-bold-title">${t("ui.difficulty")}</span>: ${env.difficulty || ""}</p>
<p class="df-env-adv-line"><span class="df-bold-title">${t("ui.potential.adversaries")}</span>: ${env.potentialAdversaries || ""}</p>
</div>
<div class="df-features-section">
<h3 class="df-features-heading">${t("ui.features")}</h3>${featuresHTML}
</div>
</div>
</section>
`.trim();
