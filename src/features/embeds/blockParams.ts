 ...
 * adversary-only, renders that many HP/stress rows (battle groups).
 */
...
export function parseEmbedParams(source: string): EmbedParams {
	const params: EmbedParams = { id: null, instance: null, count: null };

	for (const line of source.split("\n")) {
		const keyed = line.match(/^\s*(id|instance|count)\s*:\s*(\S+)\s*$/);
		if (keyed) {
...
export function buildEmbedBlock(
	language: string,
	params: { id: string; instance?: string; count?: number },
): string {
